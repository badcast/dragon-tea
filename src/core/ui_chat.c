...
GtkWidget *create_chat_widget()
{
    ...
    gtk_widget_set_margin_left(text_view, 10);
    gtk_widget_set_margin_right(text_view, 10);
    ...
    // Статус
    GtkWidget *label_status = widgets.chat_tab.label_status = gtk_label_new("");
    gtk_box_pack_start(GTK_BOX(status_box), label_status, TRUE, TRUE, 0);
    gtk_box_pack_end(GTK_BOX(status_box), gtk_image_new_from_icon_name("mail-send-receive", GTK_ICON_SIZE_SMALL_TOOLBAR), FALSE, TRUE, 10);
