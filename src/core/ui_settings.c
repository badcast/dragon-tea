...
GtkWidget *create_settings_widget()
{
    ...
    gtk_box_pack_start(GTK_BOX(widgetbox), gtk_label_new("Предпочитаемый сервер: "), TRUE, FALSE, 0);
    ...
    GtkWidget *button_apply = widgets.settings_tab.button_apply = gtk_button_new_with_label("Применить изменения");
